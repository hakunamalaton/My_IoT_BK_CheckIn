...
function FormBuilderField(props) {
    ...
    const label = field.tooltip ? (
        <span>
            {field.label}
            <Tooltip title={field.tooltip}>{/* <QuestionIcon /> */}</Tooltip>
        </span>
    ) : (
        field.label
    );
    ...
    const formItemProps = {
        ...
        className: `${
            meta.viewMode
                ? 'ant-form-item-view-mode' + (isV4 ? ' ant-form-item-view-mode-v4' : '')
                : ''
    ...
    if (isFieldViewMode) {
        ...
        if (!viewEle) {
            ...
            else if (viewValue === undefined) viewEle = '';
            else {
                viewEle = (
                    <span className="antd-form-builder-string-content">
                        {String(viewValue) || ''}
                    </span>
                );
            }
    ...
    return field.noFormItem || field.noStyle ? (
        ele2
    ) : (
        <FormItem {...formItemProps}>{ele2}</FormItem>
    );
}
